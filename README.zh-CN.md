...
- **自动备份**，在「设置」里选目录后，约每小时把 `data.json` 镜像过去
- **托盘常驻**，关窗收起、双击叫回，可设开机自启
- **设置集中管理**，语言、开机自启、自动备份都放在「设置」里，不堆到托盘菜单
- **多套主题**，内置五套浅色主题，可跟随系统明暗，也能加自定义主题文件
...
内置五套主题：暖纸、薄荷、天空、玫瑰、沙色。右键托盘图标或点标题栏的菜单按钮，在「皮肤」组里直接切换。选「跟随系统」时，按 Windows 的应用明暗模式自动切换主题。换主题时主窗口、侧边胶囊、菜单、确认框、设置窗口、主题管理窗口和行内编辑框会跟着变，托盘保持固定应用图标。
